...
for (const required of ["Complete Online Presence", "$495/month", "Google Business Profile link/access status", "Google Maps link", "Yelp link/access status", "website link/access status", "Facebook link/access status", "Instagram link/access status", "menu/order/reservation links", "best sellers", "media supply", "business-truth confirmations", "website alignment permissions", "monthly report baseline inputs", "No production auth", "real storage uploads", "live integrations", "payments", "automated customer-visible execution", "does not recommend or invent discounts"]) if (!docs.includes(required)) failures.push(`Restaurant onboarding docs missing marker: ${required}`);
