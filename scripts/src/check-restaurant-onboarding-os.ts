...
for (const required of ["I understand Veroxa does not handle", "I agree the restaurant is responsible for", "Yelp/TikTok/Reels/Ads yet", "24-hour response means review/answer/next step, not guaranteed completion", "no database write", "Weekly updates are included"]) if (!page.includes(required)) failures.push(`Client onboarding page missing expectation marker: ${required}`);
