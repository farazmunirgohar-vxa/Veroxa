...
for (const marker of ["from(\"ai_drafts\")", "restaurant_id = cleanRequired", "draft_text = cleanRequired", "cleanDraftType", "cleanSafetyFlags", "status: \"needs_review\"", "holdAiDraft", "rejectAiDraft"]) if (!service.includes(marker)) failures.push(`AI draft service missing ${marker}.`);
...
for (const marker of ["ai_drafts_active_team_select", "ai_drafts_active_team_insert", "ai_drafts_active_team_safe_update", "current_user_is_active_team()", "length(btrim(draft_text)) > 0", "media_summary", "caption_draft", "google_update_draft", "social_caption_draft", "message_reply_draft", "profile_correction_summary", "report_draft_placeholder", "next_step_recommendation", "needs_review", "approved_internal_only"]) if (!migration.includes(marker)) failures.push(`AI draft migration missing ${marker}.`);
