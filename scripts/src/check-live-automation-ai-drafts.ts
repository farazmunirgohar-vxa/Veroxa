...
const migration = read("supabase/archive/legacy_unapplied_migrations/20260616010700_ai_draft_preparation_foundation.sql");
...
const migrationAndService = migration + "\n" + service;
...
if (/needs_review|approved_internal_only/i.test(migrationAndService) || /(^|[^a-z_])array_length\(safety_flags/i.test(migrationAndService)) failures.push("AI drafts must use existing DB enum values and jsonb safety flag checks.");
