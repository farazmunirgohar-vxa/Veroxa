...
const migration = read("supabase/archive/legacy_unapplied_migrations/20260616010600_activity_log_foundation.sql");
...
for (const marker of ["restaurant_id = cleanRequired", "teamUserId", "actor_type: \"team\"", "actor_user_id", "assertActivityLogEventType", "title = cleanRequired", "visibility", "typeof input.reportEligible !== \"boolean\"", ".eq(\"visibility\", \"client_visible\")"]) if (!service.includes(marker)) failures.push(`Activity service missing ${marker}.`);
...
if (!nav.includes('/team/activity-log') || !teamPage.includes("canUseTeamActivityLog") || !teamPage.includes("Add activity note") || !teamPage.includes("teamUserId")) failures.push("Team activity page/nav must be present, gated, and use teamUserId for manual notes.");
for (const marker of ["activity_log_active_team_insert", "activity_log_active_team_select", "activity_log_active_client_visible_select", "current_user_is_active_team()", "current_user_has_active_restaurant", "actor_type = 'team'", "actor_user_id = auth.uid()", "length(btrim(title)) > 0", "visibility in", "report_eligible in (true, false)"]) if (!migration.includes(marker)) failures.push(`Activity migration missing ${marker}.`);
