...
for (const required of ["Compatibility page", "One public launch offer", "Complete Online Presence", "Included", "Coming soon / not included", "Add-ons", "Boundaries", "Back to Home", "Request Audit"]) if (!pricing.includes(required)) failures.push(`Pricing compatibility missing marker: ${required}`);
if (!pricingSource.includes("COMPLETE_ONLINE_PRESENCE_PRICE_MONTHLY = 495") || !pricing.includes("displayPrice")) failures.push("Pricing compatibility must render $495 from the source-of-truth pricing constant, not hidden marker stuffing.");
