...
function validQualityAssessment(value: unknown): boolean {
  ...
  const qualityIssues = value.qualityIssues;
  if (!exact(value, [
    ...
    Number(value.qualityScore) > 5 || !Array.isArray(qualityIssues) ||
    qualityIssues.length < 1 || qualityIssues.length > 6 ||
    qualityIssues.some((issue) => typeof issue !== "string" || !QUALITY_ISSUES.has(issue)) ||
    new Set(qualityIssues).size !== qualityIssues.length ||
    !qualityIssues.every((issue, index) => index === 0 ||
      String(qualityIssues[index - 1]) < String(issue))) return false;
  return !qualityIssues.includes("none") || qualityIssues.length === 1;
}
