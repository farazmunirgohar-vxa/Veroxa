-- Forward-only correction for the live51 Team processor. The live51 function
-- definition compiled before its duplicated audit target column was reached.
-- Replace only the function body; preserve every immutable table and ledger row.
...
begin
  ...
  if found then
    ...
    insert into public.veroxa_media_restaurant_associations_v1 (
      ...
      instruction.requested_association,
      case
        when pg_catalog.char_length(pg_catalog.btrim(
          coalesce(instruction.association_note, '')
        )) between 3 and 2000
        then instruction.association_note
...
-- Preserve every external action lock.
do $$
begin
