-- The private placement and its immutable draft-usage audit record are one
-- trusted transaction. Set the audit trigger's transaction-local capability
-- inside this RPC immediately before the insert; never rely on a prior caller.
