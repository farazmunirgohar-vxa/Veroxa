...
create trigger audit_restaurants_public_identity_guard before insert or update on public.audit_restaurants
...
begin
  if tg_table_name = 'audit_runs'
     and new.status = 'reviewed'::public.audit_run_status
     and old.status is distinct from new.status then
    if not exists (select 1 from public.audit_findings finding where finding.audit_run_id = new.id and finding.evidence_url is not null) then
    ...
    new.completed_at := coalesce(new.completed_at, now());
        ...
        and new.status = 'reviewed'::public.audit_report_status
    ...
    if char_length(btrim(new.executive_summary)) < 20 or char_length(btrim(new.priority_actions)) < 20 then
      raise exception using errcode = '23514', message = 'reviewed_report_requires_complete_summary';
    end if;
    if not exists (select 1 from public.audit_findings finding where finding.audit_run_id = new.audit_run_id and finding.evidence_url is not null) then
      raise exception using errcode = '23514', message = 'reviewed_report_requires_evidence';
    end if;
    new.reviewed_by := auth.uid();
    new.reviewed_at := now();
  elsif tg_table_name = 'audit_requests'
        and new.status = 'reviewed'::public.audit_request_status
        and old.status is distinct from new.status then
    ...
    if not exists (
      ...
      where run.audit_request_id = target_request_id and report.status = 'reviewed'::public.audit_report_status
    ) then
...
create trigger audit_runs_review_gate before update on public.audit_runs for each row execute function private.enforce_audit_review_gates();
drop trigger if exists audit_reports_review_gate on public.audit_reports;
create trigger audit_reports_review_gate before insert or update on public.audit_reports for each row execute function private.enforce_audit_review_gates();
drop trigger if exists audit_requests_review_gate on public.audit_requests;
create trigger audit_requests_review_gate before update on public.audit_requests for each row execute function private.enforce_audit_review_gates();
