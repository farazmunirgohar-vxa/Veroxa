...
test("Client and Team source preserve real links, real-image-first selection, inline previews, and prerequisite order", async () => {
  ...
  assert.match(client, /item\.associationId \? <details[\s\S]*?It is immutable[\s\S]*?create a new permission record[\s\S]*?: !item\.sourceMediaDiscarded \? <div className="momo-warning">[\s\S]*?confirming the remaining association record/, "A recorded association must be read-only while unresolved association work remains truthfully unassigned until confirmed");
...
test("Client v3 readback accepts only sanitized coherent pipeline states", async () => {
  ...
  const [sql, rightsScopeRepair, retirement, clientData] = await Promise.all([
  ...
  assert.match(
    rightsScopeRepair,
    /create or replace function public\.veroxa_momo_client_upload_status_v3\(\s*target_restaurant_id uuid\s*\)/u,
  );
  const assetRightsJoin = rightsScopeRepair.match(
