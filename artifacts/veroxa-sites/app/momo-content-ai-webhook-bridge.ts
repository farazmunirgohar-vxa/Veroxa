...
const FUNCTION_PATH = "/functions/v1/momo-content-ai-webhook-lifecycle";
...
export type MomoContentAiWebhookBridgeConfig = {
  endpoint: string;
...
export function getMomoContentAiWebhookBridgeConfig(
...
async function sign(
  ...
  try {
    ...
    throw new Error("momo_content_ai_webhook_lifecycle_signing_unavailable");
  }
}
...
async function boundedResponseText(response: Response): Promise<string> {
  return readBoundedResponseText(response, {
    ...
    minBytes: 2,
    errorMessage: "momo_content_ai_webhook_lifecycle_bridge_invalid",
  });
}

export async function invokeMomoContentAiWebhookBridge<T>(
  config: MomoContentAiWebhookBridgeConfig,
  requestBody: Record<string, unknown>,
  fetchImplementation: typeof fetch = fetch,
): Promise<T> {
  const body = JSON.stringify(requestBody);
  if (new TextEncoder().encode(body).byteLength > 300_000) {
    throw new Error("momo_content_ai_webhook_lifecycle_request_too_large");
  }
  const timestamp = Date.now().toString();
  const nonce = crypto.randomUUID();
  const signature = await sign(config.bridgePrivateKey, timestamp, nonce, body);
  ...
  try {
    ...
    throw new Error("momo_content_ai_webhook_lifecycle_bridge_invalid");
  }
