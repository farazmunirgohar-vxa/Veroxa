...
function Media({ snapshot, restaurantId, busy, run }: { snapshot: MomoClientSnapshot; restaurantId: string; busy: boolean; run: MomoClientRun }) {
  ...
  return <div className="view"><Intro eyebrow="PRIVATE MEDIA" title="Share any food image for private assessment" description="Veroxa examines only what is visibly present, creates confidence-aware tags, and keeps dish or ingredient identity uncertain unless the owner confirms it separately. Nothing is posted from this screen." />
    ...
    <form id="client-media-upload" className="momo-panel momo-form client-media-upload" onSubmit={(event) => { event.preventDefault(); if (!file) return; void run(submitUpload, "Your image and upload instruction were saved privately."); }}>
      ...
      <p className="momo-form-note">Upload the image and stop there. Veroxa and Team Faraz handle the private checks and processing; they will contact you only if a specific permission or restaurant fact is genuinely needed. Nothing is posted or connected from this screen.</p>
      <button className="primary-button" disabled={busy || !file}>{busy ? "Saving your upload…" : "Upload once and let Veroxa handle it"}</button>
    </form>
