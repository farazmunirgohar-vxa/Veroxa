...
## Ready decisions + generic-food v2 — current authority

Canonical GitHub `main` is merged PR #163 at `59b1604d887547e2804bdd6d63c97292385dcebb`. Sites v39 is live from checkout `8749a7d442d3bb068ce626a9d297b8b227493446`: 201 canonical files at `4edae9660343cda362968bd08e544ba5a154c90a902ac961365ceb32ea820292`. Production Supabase remains exact live43 at `8a49f00ab3bd6d9623100fec238939b6cb81f17d67d0e2d3a4426559c137e41c` through verified `20260808041629_repair_momo_client_v3_displayed_asset_scope.sql`. Historical v36 repository/Sites-mirror evidence `9f5d71e6487a00a9676d70dbc7022d383fd16e32f3f2a367c8d1ff7608031c90` is not the current live ledger.

Draft PR #164 is open from `agent/momo-generic-food-ready-review-v5`, based on exact main `59b1604d887547e2804bdd6d63c97292385dcebb`. Its observed opening head `b659ec307da9455c389059b29f2d6f3ab51f095e` / Git tree `9931d63dcb16a2e2e1cb7c592d2da63b4054cb60` is a draft checkpoint, not final-head workflow or review-thread evidence. Its pre-apply source identity remains 203 Sites files at `357f6b336993d2c306c102d5be2699d7145a3144041eeb9753a2a43c48fe869e` plus 44 mirrored migrations at `9cc0bba007b6a0c06edf33563fb1bc3f4650811f8f8ea1639cc58c7028ac7324`. PR #164 is unmerged; the candidate is not applied to the database, not published to Sites, and not activation evidence.

The sole pending source migration is provisional `20260808045812_momo_ready_team_decisions_and_food_tags_v2.sql`, mirrored byte-for-byte at SHA-256 `9cf6f0080d38d58d3c1939d928444701b1954bf5cfe96bf7f3e80077bad45cc0`. Supabase MCP generates the production ledger version and cannot promise `045812`. Apply only these exact reviewed bytes, capture the generated version, then immediately merge a terminal follow-up PR that renames both source mirrors to that version without changing SQL bytes and refreshes the live, candidate, and Sites fingerprints. Only the exact-byte payload—not the provisional filename—is stable across the apply boundary.

The zero-unsafe-window sequence is: merge the reviewed feature PR; freeze all new content/package reservations plus verified-upload auto-advance and dispatch ingress; drain v4 nonterminal runs and staged webhook/outbox/unsettled-cost work to zero; apply exact SQL bytes; verify DB v5 and legacy-v4 boundaries; reconcile the generated migration version in GitHub; publish and verify the exact terminal Sites source; then restore only the prior bounded internal ingress. Live Sites v39 parses/dispatches v4 while the migration switches new reservations to v5, so both an unfrozen DB-first rollout and a Sites-first rollout are unsafe.

Scoped database and Sites deployment permission exists, while automatic deployment remains disabled and every rollout step remains pending. Applied `02609` retains the known non-comprehensive ACL residual because live `postgres` is not a `supabase_admin` member. Provider connections, external/public writes, scheduling, posting, review replies, website writes, owner authority, and Momo activation stay false.

## Historical database-repair / corrective-Sites checkpoint
