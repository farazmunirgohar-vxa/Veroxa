...
- Current state: Sites version 15 retains the deployed Auth/RLS foundation plus a confirmed, signed-in, active Team/Momo identity, the earlier verified Safari protected-route smoke, secure-email-link recovery, and user-confirmed approved-user password sign-in. A separate explicit-ack Momo client provisioning command and mock fixture are production-available but have not been run. No Momo client identity is approved or provisioned.
