...
Current build: the seven-system Momo operating foundation is applied through eight production migrations. The login repair is merged through PR #141 at `46d01c44f0411a4e870cd490d5bfcd8e58ee0e59` and deployed as verified Sites version 7. Faraz's Gmail Team identity and protected Team/Momo browser route are verified. The password-login extension is an unmerged, undeployed candidate. Momo remains operationally blocked until owner-confirmed truth, permissioned media, provider access, and remaining operating evidence are complete.
...
- Deployed Sites version 7 implements signed Supabase sessions, server route guards, active profile plus active Momo membership checks, RLS, private restaurant storage, secure-email-link sign-in, and callback safety. The unmerged candidate adds approved-user password sign-in and protected password replacement. Public signup remains disabled.
...
- Exact merged runtime source is deployed as Sites version 7. The checkpoint and custom live domain are verified active with no reported domain error.
...
- Real Team Auth provisioning, confirmation, initial sign-in, and protected Team/Momo browser smoke are complete for Faraz's approved Gmail identity without exposing a privileged key. Password setup/login remains a separate candidate gate until merged, deployed, set through the protected screen, and verified.
