...
- Current boundary: the Gmail Auth user, confirmation, real sign-in, Team profile, Momo membership, production callback, token/session exchange, and protected Team/Momo Safari route are verified. PR #141 is merged and deployed as Sites version 7.
- Password candidate: approved-user password login and protected password replacement are implemented and tested locally but remain unmerged and undeployed. The 24-hour recent-sign-in check is a portal UI guard; native hosted reauthentication must be enabled and verified separately for Auth-boundary enforcement.
