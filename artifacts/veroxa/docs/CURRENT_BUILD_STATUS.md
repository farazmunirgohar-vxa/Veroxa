...
## 2026-08-09 — internal AI active; external actions held (current authority)

- **Release-bearing GitHub identity:** commit `2721545d5823dbd4cbc233e7473d25393f4ff0ec` from merged PR #169 is the immutable activation and deployed-source identity. Its exact reviewed head `78c1f90d1412375b136aa18adcf9c4c1addc781e` and tree `37cbc6d7815dde6984b2691006ac3977df1a7226` passed CI `31296663350`, Sites `31296663318`, Supabase `31296663330`, and Veroxa `31296663326` with zero review threads. An evidence-only closeout may advance repository `main` without changing that identity or any Sites source.
- **Production database:** Supabase is live49 through generated `20260809051616_guarded_internal_ai_activation_v1.sql`, 24,248 canonical bytes at SHA-256 `22d5e82f683c3dd9d4b3d9c5b4e5003cf3a769f67dde340e98deee3ba3afb8ba`. The exact activation routine remains postgres-owned, security-definer, empty-search-path, and inaccessible to anon, authenticated, and service-role callers.
- **Guarded activation:** the routine was invoked exactly once at `2026-08-09T05:35:42.103503Z`, bound to merged GitHub `2721545d`, Sites v41/source `766ba3bc`/`96ab0a58`/archive `74c287e6`, and JWT-protected Edge v7/bundle `a6b00feeab795faa91d6d8d015c4ad399c526e1b35f702778a8c55aaba49503d`. Audit event `d31dc513-f953-4aca-9746-3f69447a6ae8` records those identities. The one-shot gate and rollout authorization are consumed; this checkpoint authorizes no repeat invocation or further runtime deployment.
- **Runtime and ACL result:** `ai_live_calls=true`; `provider_writes=false`, `review_replies=false`, `website_writes=false`, and `external_scheduling=false`. Postflight found 59 registered functions, zero missing, zero grant mismatches, zero anon grants, exactly 13 authenticated and 32 service-role grants restored from source, and 14 functions still held.
- **Second runtime parity:** Sites v41 is live at `veroxasystems.com` with version ID `appgprj_6a53d07c7c28819182801cf35dfd30de~appgver_36b5c80ee2a48191acf5bcf809fd8ad7`, source commit `766ba3bc2a7ebd68c1d72ae7f53d159d2edca593`, exact 216-file source SHA-256 `96ab0a58d24c59ce176e3362730897764d039fdc2c3f8bd14d65317d1992532b`, and archive SHA-256 `74c287e655495edde605f0fc38ebc06f1ed0f19275d550c4369e491430f7cea7`. Edge v7 remains exact and ACTIVE.
- **Postflight and cost:** relevant work, outbound HTTP, cost-ledger rows, provider-called ledger rows, accounted microusd, and bounded post-activation Edge invocations are all zero; incremental spend is USD 0. A rollback-only authenticated-role smoke proved one active Team profile, one active Momo membership, all three bounded read RPCs executable, activation execution and direct candidate INSERT denied, 0 Ready rows, 2 upload-status rows all external-locked, and 0 media-window rows. Prior live cross-tenant denial evidence remains authoritative because no second live restaurant existed for a fresh negative target.
- **Release closeout:** the scoped internal-AI rollout is complete. No further Sites or Edge deployment is required for this evidence-only closeout; GitHub/Sites/Edge/database release identities remain exact, and no Sites v42 is required.
- **Authorization boundary:** Faraz authorized this scoped internal rollout up to USD 20, with a USD 0 incremental spend target. That rollout authorization is consumed. External providers, provider writes, review replies, website writes, scheduling, posts, owner contact, and public writes remain locked.
- **Proof boundary:** no real upload, provider call, Ready decision, owner confirmation, current-offering association, external action, owner contact, or public publication is claimed. Internal activation is proven only by the audited database event and postflight evidence.
...
- **Production now / repository lineage:** Sites v37 is live from `61e9ace7723ef56f42111f320327187596406944` (200 files, `929e05cf68a6af5176811f49321ec108e617b93a08153b65b3f86b109d0c8c18`). The exact live database has 43 migrations at `8a49f00ab3bd6d9623100fec238939b6cb81f17d67d0e2d3a4426559c137e41c`, through `20260808041629_repair_momo_client_v3_displayed_asset_scope.sql`. PR #162 is already merged as canonical `main` `ca47aeff7ab44a69b6ce039608ae27fea6c3c326` and the candidate base; no Sites or database advancement is implied.
- **Applied-history boundary:** `20260808001430_momo_client_pipeline_readback_v3.sql` was applied at immutable SHA-256 `987186e74590c6e484ebfee47e1c7ed384e2b4dc8c4a97ad7243ae38feb765cc`. Source must preserve those exact bytes; the displayed-asset change belongs only in applied forward migration `041629`, 7,293 bytes at `6cbf3f80d028d3fe54093b14bae59314913b4f0bfacfbf31fce4aa2a24e429ba`.
- **Candidate evidence:** branch `agent/momo-client-v3-forward-scope-repair` has 201 Sites files at `4edae9660343cda362968bd08e544ba5a154c90a902ac961365ceb32ea820292` and 43 mirrored migrations at `8a49f00ab3bd6d9623100fec238939b6cb81f17d67d0e2d3a4426559c137e41c`. PR #163 is open but not merged; exact-final-head workflows and review-thread evidence remain unrecorded. Local review passed, the migration tree matches live43, and corrected Sites v38 is not live.
- **Rollout status:** `01210`, immutable `01430`, Sites v37, `01842`, `01853`, `02609`, and verified `041629` are complete. No database migration remains pending. Production authority is granted, but automatic deployment is disabled: pass local and hosted gates, then publish/verify Sites v38.
- **Residual safety:** applied `02609` skipped `supabase_admin` because live `postgres` is not a member, so it is not comprehensive default-ACL closure. Provider connection, external publishing authority, and Momo activation remain false.
...
- **Safety:** queues, schedules, provider connections, and external writes remain empty/off. Verified GitHub parity authorizes no scheduling, publishing, posting, provider canary, deployment, database apply, or activation.
...
- Observed live database: 11 applied migrations. Migration 11 is `20260713222721_upgrade_restaurant_audit_engine_v3_partial_scoring.sql`, SHA-256 `304eb98db628b09fa245fba156160b043c1ba9ba2f9aeb689086a6a18ad234b2`.
- Production is ahead of canonical source. The current worktree imports the exact Audit V3/Sites and migration evidence plus release controls. The reconciliation candidate is unmerged and undeployed: it is not republished or applied to production.
- `VEROXA_DEPLOYMENT_MANIFEST.json` records the current freeze, disabled activation state, observed baseline, candidate tree fingerprints, and deferred cleanup gates. CI—not a hand-written document—must bind the eventual reviewed commit through an exact-`GITHUB_SHA` attestation.
- Product deployments remain frozen except the exact green, reviewed, merged reconciliation release. The candidate does not authorize AI, credentials, Momo/client contact, Client provisioning, activation, providers, publishing, billing, external writes, or new spend.
- Momo visual/manual review may continue against honest blocked and empty states; the onboarding gate remains No-Go until real, same-owner evidence satisfies it and Faraz explicitly authorizes contact.
- Vercel's shutdown sentinel remains. External Git disconnection, branch pruning, and legacy Vite removal are blocked/deferred until post-release parity, rollback evidence, ownership classification, and explicit approval.
- Older PR #145 / Supabase 10 / Sites v11 / Audit V2 material below is retained as release history and is not current live truth.
...
- Release state: PR #142 is merged to `main` at `9a905c822f084fd2df5c9a2cb87c1a8286647e59`; its exact source is deployed as verified Sites version 8 with all eight production migrations still applied and both custom domains active.
- Current boundary: the Gmail Auth user, confirmation, real sign-in, Team profile, Momo membership, production callback, token/session exchange, protected Team/Momo route, approved-user password sign-in, and protected password-replacement surface are deployed. Faraz confirmed password sign-in works. Secure-email-link access remains the recovery path and public signup remains disabled.
- Unverified security follow-up: the 24-hour `last_sign_in_at` check is a portal-side recent-sign-in guard. Hosted reauthentication behavior and rejection of an old session after password replacement have not been verified; do not infer either from the successful password login. Old-session sign-out failure is currently best-effort and must remain an explicit follow-up.
- Readiness authority: `momo-readiness-tracker.json` is the repository release baseline/checkpoint. The scoped Supabase readiness rows and readiness summary RPC are authoritative for the live Team operational view; the two records must not be represented as one automatically synchronized source.
...
- Historical external blocker at PR #138: Team provisioning was then blocked. That Team blocker is closed by the PR #142 identity evidence; no Momo client identity is approved or provisioned.
...
- Implemented in deployed release source: server-verified Supabase sessions, active profile/membership checks, Team/Momo RLS, private Momo media storage policies, secure-email-link sign-in, callback safety, and safe-empty client routes. The current unmerged candidate adds approved-user password sign-in and portal-guarded password replacement. Public signup remains disabled; a fresh email-link session is the recovery path.
...
- Advisor truth: current Momo/Audit hardening has no known error-level issue. Intentional warnings remain for scoped membership helpers and signed public intake; six mutable-search-path warnings and legacy-table performance notices belong to older unused schema. Supabase Free native leaked-password enforcement remains off; the password candidate adds a padded HIBP partial-hash check as bypassable client defense in depth and does not enable public signup.
...
- Historical PR #135 Auth boundary: public Auth user creation was disabled and Team pre-provisioning was still pending at that release. Current PR #142 evidence verifies Faraz's approved Team identity; no Momo client identity or real Momo data is active.
