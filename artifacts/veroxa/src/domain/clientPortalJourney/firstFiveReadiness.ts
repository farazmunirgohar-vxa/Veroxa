...
function getMediaRisk(
  ...
  if (fixture.usableMediaCount < 6 || fixture.uploadConsistency === "low")
    return "low_media_risk";
  if (fixture.uploadConsistency === "inconsistent")
    return "inconsistent_uploads";
  return "healthy_supply";
}

function getContentQueueState(
  fixture: FirstFiveClientReadinessFixture,
): ContentQueueState {
  if (fixture.usableMediaCount < 6) return "needs_media";
  if (fixture.uploadConsistency === "inconsistent") return "hold_for_later";
  if (
    (fixture.plan === "growth" || fixture.plan === "premium") &&
    fixture.hasShortVideoContent
  )
    return "ready_for_reels";
  return "ready_for_content";
}

function getReportReadinessState(
  fixture: FirstFiveClientReadinessFixture,
): ReportReadinessState {
  if (!fixture.weeklyUpdateReady || !fixture.monthlyReportReady)
    return "report_needs_review";
  return fixture.monthlyReportReady
    ? "monthly_report_ready"
    : "weekly_update_ready";
}

function getMediaSupplyStatus(
  mediaRiskLevel: MediaRiskLevel,
  usableMediaCount: number,
): string {
  if (mediaRiskLevel === "low_media_risk")
    return `More content needed — ${usableMediaCount} usable items available.`;
  if (mediaRiskLevel === "inconsistent_uploads")
    return `Usable content exists, but uploads are inconsistent.`;
  return `Content supply looks workable — ${usableMediaCount} usable items available.`;
}

function getNextHelpfulAction(
  fixture: FirstFiveClientReadinessFixture,
  mediaRiskLevel: MediaRiskLevel,
): string {
  if (mediaRiskLevel === "low_media_risk")
    return "Send a few fresh food photos or short videos so posting does not slow down.";
  if (mediaRiskLevel === "inconsistent_uploads")
    return "Send one small batch of fresh media this week so Veroxa can keep work moving.";
  if (fixture.condition === "premium_assessment_eligible")
    return "Premium can be reviewed with Veroxa when you are ready; ad spend stays separate.";
  if (fixture.plan === "growth" && fixture.hasShortVideoContent)
    return "Keep sending usable short videos when you have them.";
  return "Keep sharing fresh photos when you have new dishes, specials, or busy moments.";
}

function getNextTeamAction(
  model: Pick<
  ...
  if (model.mediaRiskLevel === "low_media_risk")
    return "Send calm media request with simple examples.";
  if (model.mediaRiskLevel === "inconsistent_uploads")
    return "Recommend a weekly upload rhythm and hold non-urgent drafts.";
  if (model.premiumCandidate)
    return "Offer Premium readiness assessment; do not start ads without approval and budget.";
  if (model.reportReadinessState === "report_needs_review")
    return "Review report inputs before monthly snapshot is shown.";
  if (model.contentQueueState === "ready_for_reels")
    return "Prepare short-video draft for review using provided media.";
...
function getRiskReason(
  mediaRiskLevel: MediaRiskLevel,
  premiumCandidate: boolean,
  reportState: ReportReadinessState,
): string {
  if (mediaRiskLevel === "low_media_risk")
    return "Posting may slow because usable media is low.";
  if (mediaRiskLevel === "inconsistent_uploads")
    return "Client uploads are uneven, so queue timing needs human review.";
  if (premiumCandidate)
    return "Premium assessment is possible, but ads require explicit approval and agreed ad budget.";
  if (reportState === "report_needs_review")
    return "Report is not ready to show without Veroxa review.";
...
export function buildFirstFiveClientReadinessModel(
  ...
  const premiumCandidate =
    fixture.condition === "premium_assessment_eligible" &&
    premiumEligibilityStatus === "eligible_for_assessment";
  ...
  return {
    ...
    mediaRiskLevel,
    mediaSupplyStatus: getMediaSupplyStatus(
      mediaRiskLevel,
      fixture.usableMediaCount,
    ),
    contentQueueState,
    reportReadinessState,
    ...
    recommendedHumanFollowUp:
      mediaRiskLevel === "healthy_supply"
        ? "No urgent follow-up; keep normal review rhythm."
        : "Human review recommended before promising timing.",
    ...
    weeklyUpdateStatus: fixture.weeklyUpdateReady
      ? "Weekly update prepared"
      : "Weekly update in review",
    monthlyReportStatus: fixture.monthlyReportReady
...
export function mapFirstFiveModelToTeamView(
  ...
  return {
    ...
    packageLabel: model.planLabel,
    mediaRiskLevel:
      model.mediaRiskLevel === "healthy_supply"
        ? "Healthy supply"
        : model.mediaRiskLevel === "low_media_risk"
          ? "Needs media"
          : "Inconsistent uploads",
    contentQueueState:
      model.contentQueueState === "ready_for_reels"
        ? "Ready for content"
        : model.contentQueueState.replaceAll("_", " "),
...
export function getFirstFiveReadinessModels(): FirstFiveClientReadinessModel[] {
  return FIRST_FIVE_CLIENT_READINESS_FIXTURES.map(
...
export function getFirstFiveTeamCommandCenterSummary(): FirstFiveTeamCommandCenterSummary {
  ...
  const clientsNeedingMedia = models.filter(
    (m) => m.mediaRiskLevel === "low_media_risk",
  ).length;
  const inconsistentUploads = models.filter(
    (m) => m.mediaRiskLevel === "inconsistent_uploads",
  ).length;
  const clientsReadyForContent = models.filter(
    (m) =>
      m.contentQueueState === "ready_for_content" ||
      m.contentQueueState === "ready_for_reels",
  ).length;
  const reportsNeedingReview = models.filter(
    (m) => m.reportReadinessState === "report_needs_review",
  ).length;
  const premiumAssessmentCandidates = models.filter(
    (m) => m.premiumCandidate,
  ).length;
  ...
  return {
    ...
    internalAlerts: models
      .filter(
        (m) =>
          m.mediaRiskLevel !== "healthy_supply" ||
          m.premiumCandidate ||
          m.reportReadinessState === "report_needs_review",
      )
