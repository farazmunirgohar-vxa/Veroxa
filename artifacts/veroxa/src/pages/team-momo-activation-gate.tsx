...
export default function TeamMomoActivationGate() {
  ...
  if (!canUse) {
    return <PortalLayout items={teamPortalNavItems} portalName="Team Portal"><RealPortalReviewNotice /><SafePortalEmptyCard title="Controlled Momo Pilot Activation Gate in review" body="This Team-only activation-decision gate requires real auth, the Momo activation gate feature flag, and an authenticated Team role. Placeholder mode stays honest and empty with no fake activation readiness, no credential creation, no external contact, no platform connection, and no pilot activation." testId="empty-team-momo-activation-gate" /><BoundaryCopy /><TeamReviewModeRouteSummary title="Controlled Momo Pilot Activation Gate review-mode summary" /></PortalLayout>;
  }
  ...
  return <PortalLayout items={teamPortalNavItems} portalName="Team Portal"><div className="mb-4 flex items-start justify-between gap-3"><div><h2 className="text-2xl font-bold tracking-tight" data-testid="header-team-momo-activation-gate">Controlled Momo Pilot Activation Gate</h2><p className="mt-1 text-sm text-muted-foreground">Internal Team-only decision gate that reads existing readiness evidence and blockers. This does not perform activation.</p></div><Badge variant="outline">Team only</Badge></div><BoundaryCopy />
    ...
    <Card className="mt-4"><CardHeader><CardTitle className="flex items-center gap-2 text-sm"><CheckCircle2 className="h-4 w-4" />Final Faraz decision required</CardTitle></CardHeader><CardContent className="grid gap-3">{finalItems.map((entry) => <GateItemCard key={entry.id} item={entry} />)}<p className="text-sm text-muted-foreground">Momo owner walkthrough remains blocked until Faraz approval.</p><div className="flex flex-wrap gap-2 pt-2">{internalLinks.map((href) => <Button asChild key={href} size="sm" variant="outline"><Link href={href}>{href.replace("/team/", "")}</Link></Button>)}</div></CardContent></Card><TeamReviewModeRouteSummary title="Controlled Momo Pilot Activation Gate safe checklist summary" /></PortalLayout>;
}
