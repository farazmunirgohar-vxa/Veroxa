...
import {
  getMomoLivePilotReadiness,
  type MomoLivePilotReadiness,
  type MomoReadinessItem,
} from "@/lib/momoReadiness/momoReadinessService";
...
const fallback: MomoLivePilotReadiness = {
  summary: {
    ...
    <Card>
      <CardContent className="p-4">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          <strong>Safe next step:</strong> {item.action_hint}
        </p>
        {item.route_href ? (
          <Button asChild className="mt-3" size="sm" variant="outline">
            <Link href={item.route_href}>Open existing Team page</Link>
          </Button>
        ) : null}
      </CardContent>
    </Card>
...
export default function TeamMomoLiveReadiness() {
  ...
  useEffect(() => {
    ...
    if (!client) return;
    void getMomoLivePilotReadiness(client)
      .then(setReadiness)
      .catch(() => setReadiness(fallback));
  }, [canUse]);
  ...
  if (!canUse) {
    return (
      <PortalLayout items={teamPortalNavItems} portalName="Team Portal">
        <RealPortalReviewNotice />
        <SafePortalEmptyCard
          title="Momo Live Pilot Readiness Gate in review"
          body="This Team-only readiness gate requires real auth, the Momo readiness feature flag, and an authenticated Team role. Placeholder mode stays honest and empty with no fake completed readiness, no live pilot change, and no external action."
  ...
  const ownerItems = readiness.checklist.filter(
    (entry) => entry.status === "needs_owner_confirmation",
  );
  const moduleItems = readiness.checklist.filter(
    (entry) =>
      !["Activation Boundaries", "Restaurant Identity"].includes(
        entry.category,
      ),
  );
  const foundations = readiness.checklist.filter((entry) =>
    [
      "Restaurant Identity",
      "Client Access Readiness",
  ...
  return (
    <PortalLayout items={teamPortalNavItems} portalName="Team Portal">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <h2
            ...
            data-testid="header-team-momo-live-readiness"
          >
            Momo Live Pilot Readiness Gate
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Team-only internal checklist for controlled activation consideration
            after Live Automation V1 foundations.
          </p>
        </div>
      ...
      <section className="mt-4 grid gap-3 md:grid-cols-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="mt-2 text-2xl font-bold">
              {readiness.summary.blocker_count}
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            ...
            <p className="mt-2 text-2xl font-bold">
              {readiness.summary.owner_confirmation_count}
            </p>
          </CardContent>
        </Card>
        <Card>
        ...
        <CardContent className="grid gap-3">
          {readiness.blockers.length ? (
            readiness.blockers.map((entry) => (
              <ReadinessItemCard key={entry.id} item={entry} />
            ))
          ) : (
            <p className="text-sm text-muted-foreground">
              No blocker records returned by the real readiness check.
            </p>
          )}
        </CardContent>
      </Card>
      <Card className="mt-4">
        ...
        <CardContent className="grid gap-3">
          {ownerItems.length ? (
            ownerItems.map((entry) => (
              <ReadinessItemCard key={entry.id} item={entry} />
            ))
          ) : (
            <p className="text-sm text-muted-foreground">
              No owner-confirmation items returned by the real readiness check.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="mt-4">
        <CardHeader>
          <CardTitle className="text-sm">Activation boundaries</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-2 text-sm text-muted-foreground">
          <p>
            PR #109 readiness gate, PR #110 readiness alignment, and PR #111
            Controlled Momo Pilot Activation Gate are already merged.
          </p>
          <p>
            Future real-world activation requires separate explicit Faraz
            approval.
          </p>
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {[
              ...
              <Button asChild key={href} size="sm" variant="outline">
                <Link href={href}>{href.replace("/team/", "")}</Link>
              </Button>
            ))}
          </div>
        </CardContent>
