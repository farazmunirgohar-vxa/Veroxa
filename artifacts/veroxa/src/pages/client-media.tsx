...
function MediaStatusBadge({ status }: { status: ClientMediaDisplayStatus }) {
  return (
    <Badge variant="outline" className={`text-[10px] ${toneStyles[getClientMediaStatusTone(status)]}`}>
      {status}
    </Badge>
  );
}
...
export default function ClientMedia() {
    ...
    return clientTeamWorkRepository
      ...
      .map((submission) => {
        ...
        const status = normalizeClientMediaDisplayStatus(
          submission.status === "blocked" ? "Needs better media" : (workItem?.statusLabel ?? submission.status),
        );
        return {
          id: submission.id,
  ...
  const handleSubmitToTeam = async () => {
    ...
    const item = createWorkflowItem({
      ...
      type: "media_upload",
      title: files.length > 1 ? `${files.length} media items from your restaurant` : files[0].name,
  ...
  const handlePick = (event: ChangeEvent<HTMLInputElement>) => {
    const picked = Array.from(event.target.files ?? []).map<SelectedFile>((file) => ({
      name: file.name,
    ...
    setPickerMessage("Added. Direction is optional — you can submit without it.");
    if (inputRef.current) inputRef.current.value = "";
  };

  const removeFile = (index: number) => setFiles((prev) => prev.filter((_, fileIndex) => fileIndex !== index));
  ...
  return (
    <PortalLayout items={clientPortalNavItems} portalName="Client Portal">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-3xl font-bold tracking-tight" data-testid="header-client-media">Media</h2>
          <p className="mt-1 max-w-2xl text-sm md:text-base text-muted-foreground">
            Drop off photos and videos, then track what is uploaded, ready, or posted.
          </p>
        </div>
        <Button type="button" onClick={() => inputRef.current?.click()} data-testid="button-quick-upload">
          <UploadCloud className="mr-2 h-4 w-4" /> Upload media
        </Button>
      </div>

      <Card className="border-primary/30 bg-card" data-testid="card-upload-dropoff">
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-base">
            <UploadCloud className="h-4 w-4 text-primary" /> Upload media
          </CardTitle>
          <p className="text-xs text-muted-foreground">Add a quick note if Veroxa should know a dish, event, or preference.</p>
        </CardHeader>
        <CardContent className="space-y-3">
          <input ref={inputRef} type="file" multiple accept="image/*,video/*" className="hidden" onChange={handlePick} data-testid="input-files" />
          <div className="rounded-lg border border-dashed border-border bg-muted/10 p-5 text-center">
            ...
            <p className="text-sm font-medium">Choose restaurant photos or videos</p>
            <p className="mb-3 text-xs text-muted-foreground">Raw phone media is fine.</p>
            <Button type="button" variant="outline" onClick={() => inputRef.current?.click()} data-testid="button-choose-files">
              Choose files
            </Button>
            {pickerMessage && <p className="mt-3 text-xs text-amber-300" role="status">{pickerMessage}</p>}
          </div>

          {files.length > 0 && (
            <div className="space-y-3 rounded-md border border-primary/30 bg-primary/5 p-3" data-testid="pending-upload-panel">
              <div className="grid gap-2 md:grid-cols-[minmax(0,1fr)_220px]">
                ...
                >
                  {DIRECTION_OPTIONS.map((option) => <option key={option.label} value={option.value}>{option.label}</option>)}
                </select>
              </div>
              <ul className="space-y-1.5">
                {files.map((file, index) => (
                  <li key={`${file.name}-${index}`} className="flex items-center justify-between gap-3 rounded-md border border-border bg-card/70 px-3 py-2 text-sm" data-testid={`selected-file-${index}`}>
                    <span className="truncate">{file.name} <span className="text-xs text-muted-foreground">· {file.sizeKb} KB</span></span>
                    <button type="button" onClick={() => removeFile(index)} className="text-muted-foreground hover:text-foreground" aria-label={`Remove ${file.name}`}>
                      <X className="h-4 w-4" />
                    </button>
                  </li>
                ))}
              ...
              <div className="flex justify-end">
                <Button type="button" onClick={handleSubmitToTeam} data-testid="button-submit-media">
                  <Send className="mr-2 h-4 w-4" /> Submit media
                </Button>
              </div>
            </div>
          )}
          {submitMessage && <p className="rounded-md border border-emerald-500/20 bg-emerald-500/5 px-3 py-2 text-sm text-emerald-200" role="status">{submitMessage}</p>}
        </CardContent>
      </Card>

      <div className="grid gap-4 lg:grid-cols-3">
        <MediaSection title="Uploaded Media" items={uploadedMedia} empty="New uploads and items waiting for review appear here." />
        <MediaSection title="Ready Media" items={readyMedia} empty="Reviewed media that is ready or scheduled appears here." />
        <MediaSection title="Posted Media" items={postedMedia} empty="Posted or already-used media appears here." />
      </div>
    </PortalLayout>
  );
}

function MediaSection({ title, items, empty }: { title: string; items: ClientMediaItem[]; empty: string }) {
  return (
    <Card className="border-border bg-card" data-testid={`section-${title.toLowerCase().replaceAll(" ", "-")}`}>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base"><Inbox className="h-4 w-4 text-primary" /> {title}</CardTitle>
      </CardHeader>
      <CardContent>
        {items.length === 0 ? (
          <SafePortalEmptyCard title={title} body={empty} testId={`empty-${title.toLowerCase().replaceAll(" ", "-")}`} />
        ) : (
          <ul className="space-y-2">
            {items.map((item) => (
              <li key={item.id} className="rounded-md border border-border bg-muted/20 px-3 py-3" data-testid={`media-item-${item.id}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium">{item.name}</p>
                    <p className="mt-0.5 line-clamp-2 text-xs text-muted-foreground">{item.note || item.description}</p>
                  </div>
                  <MediaStatusBadge status={item.status} />
                </div>
                {item.direction && <p className="mt-2 text-xs text-muted-foreground">Direction: {item.direction}</p>}
              </li>
            ))}
          </ul>
        )}
