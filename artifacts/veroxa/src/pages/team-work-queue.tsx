...
import { demoClientTeamWorkflow, type WorkflowItem } from "@/data/workflows/clientTeamWorkflow";
import { WorkflowItemCard, type WorkflowItemCardAction } from "@/components/workflows/WorkflowItemCard";
import {
...
type LocalDecision = "pending" | "reviewed" | "ask_client" | "queue_later" | "hold_later";
...
function actionsForItem(item: WorkflowItem, setDecision: (id: string, decision: LocalDecision) => void): WorkflowItemCardAction[] {
  if (item.stage === "needs_better_photo" || item.stage === "needs_client_action") {
    return [
      { label: "Ask client", tone: "warn", onClick: () => setDecision(item.id, "ask_client") },
      { label: "Hold for later", onClick: () => setDecision(item.id, "hold_later") },
    ];
  }

  if (item.stage === "scheduled") {
    return [
      { label: "Queue for later", tone: "primary", onClick: () => setDecision(item.id, "queue_later") },
      { label: "Hold for later", onClick: () => setDecision(item.id, "hold_later") },
    ];
  }

  return [
    { label: "Mark reviewed", tone: "primary", onClick: () => setDecision(item.id, "reviewed") },
    { label: "Ask client", tone: "warn", onClick: () => setDecision(item.id, "ask_client") },
    { label: "Queue for later", onClick: () => setDecision(item.id, "queue_later") },
  ];
}
...
export default function TeamWorkQueue() {
  ...
  const summaryTiles = [
    ...
    { label: "Needs review", value: summary.teamReviewReady, testId: "wq-summary-review" },
    { label: "Waiting on client", value: summary.waitingOnClient, testId: "wq-summary-waiting" },
    { label: "Queue / hold later", value: summary.readyToQueueOrHold, testId: "wq-summary-queue" },
    { label: "Alerts", value: summary.alerts, testId: "wq-summary-alerts" },
    { label: "Completed", value: summary.completed, testId: "wq-summary-completed" },
  ];
    ...
    <PortalLayout items={teamPortalNavItems} portalName="Team Portal">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3 mb-4">
        {summaryTiles.map((tile) => (
          <Card key={tile.label} className="bg-card border-border" data-testid={tile.testId}>
      ...
      <div className="mb-4 rounded-md border border-border bg-muted/10 px-3 py-2 text-xs text-muted-foreground">
        ...
        Mark reviewed · Ask client · Queue for later · Hold for later. These are review notes only in this demo.
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {groups.map((group) => (
          <Card key={group.key} className="bg-card border-border" data-testid={`work-group-${group.key}`}>
            ...
            <CardContent className="space-y-2">
              {group.items.length === 0 ? (
                ...
                group.items.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="space-y-1.5" data-testid={`work-item-${item.id}`}>
                      <WorkflowItemCard
                        item={item}
                        mode="team"
                      ...
                      <div className="flex items-start justify-between gap-2 px-1">
                        ...
                        <span className={`rounded border px-2 py-0.5 text-[10px] font-medium whitespace-nowrap ${decisionTone[decision]}`}>
