...
import { AI_DRAFT_SAFETY_FLAGS, AI_DRAFT_SOURCE_ENTITY_TYPES, AI_DRAFT_TYPES, createAiDraftRecord, holdAiDraft, listAiDraftsForTeam, rejectAiDraft, updateAiDraftStatus, type AiDraftSafetyFlag, type AiDraftSourceEntityType, type AiDraftType } from "@/lib/aiDrafts/aiDraftService";
...
export default function TeamAiDrafts() {
  ...
  async function setItemStatus(item: AiDraftRecord, next: "held" | "rejected" | "approved_internal_only") {
    const client = getSupabaseClient();
    if (!client) return;
    try {
      const updated = next === "held" ? await holdAiDraft(client, item.id, item.restaurant_id) : next === "rejected" ? await rejectAiDraft(client, item.id, item.restaurant_id) : await updateAiDraftStatus({ client, draftId: item.id, restaurantId: item.restaurant_id, status: next });
