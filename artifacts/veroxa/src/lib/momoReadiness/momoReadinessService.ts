...
export async function getMomoReadinessChecklist(client: SupabaseClient): Promise<MomoReadinessItem[]> {
  ...
  const [profileFields, media, messages, corrections, activity, drafts, reports, approvals] = await Promise.all([
    restaurantId ? maybeRowsForRestaurant(client, "restaurant_profile_fields", restaurantId, "id,restaurant_id,section,label,value,status,created_at", 50) : Promise.resolve([]),
    restaurantId ? maybeRowsForRestaurant(client, "media_assets", restaurantId, "id,status,restaurant_id,created_at", 50) : Promise.resolve([]),
    restaurantId ? maybeRowsForRestaurant(client, "messages", restaurantId, "id,status,restaurant_id,created_at", 50) : Promise.resolve([]),
    restaurantId ? maybeRowsForRestaurant(client, "profile_corrections", restaurantId, "id,status,restaurant_id,field_id,field_label,created_at", 50) : Promise.resolve([]),
    restaurantId ? maybeRowsForRestaurant(client, "activity_log", restaurantId, "id,restaurant_id,report_eligible,visibility,created_at", 50) : Promise.resolve([]),
    restaurantId ? maybeRowsForRestaurant(client, "ai_drafts", restaurantId, "id,status,restaurant_id,safety_flags,created_at", 50) : Promise.resolve([]),
    restaurantId ? maybeRowsForRestaurant(client, "reports", restaurantId, "id,status,restaurant_id,created_at", 50) : Promise.resolve([]),
    restaurantId ? maybeRowsForRestaurant(client, "approvals", restaurantId, "id,status,restaurant_id,created_at", 50) : Promise.resolve([]),
  ]);
  const confirmedProfileFields = profileFields.filter((field) => ["confirmed", "pre_filled", "veroxa_review"].includes(String(field.status)) && String(field.value ?? "").trim().length > 0);
  const hasTruth = Boolean(restaurant?.address && restaurant?.phone && (restaurant?.timezone || confirmedProfileFields.length));
  ...
  return [
    ...
    item({ id: "business-truth", category: "Restaurant Identity", title: "Business-truth fields", status: hasTruth ? "needs_review" : "needs_owner_confirmation", severity: hasTruth ? "warning" : "critical", description: "Name, address, phone, hours, menu, and brand identity must be confirmed before public use.", evidence: hasTruth ? `${confirmedProfileFields.length} confirmed/prepared profile field(s) found with restaurant identity basics.` : "Required business-truth fields are missing or not readable.", action_hint: "Business-truth changes still require owner confirmation.", route_href: "/team/profile-corrections" }),
    ...
    item({ id: "media", category: "Media Readiness", title: "Media foundation and assets", status: media.length ? "needs_review" : "not_ready", severity: media.length ? "warning" : "critical", description: "Media upload/storage foundation can be checked for existing Momo assets only.", evidence: `${media.length} Momo media asset record(s) found for review scope.`, action_hint: media.length ? "Review media status in the upload inbox; do not post automatically." : "Request usable media only through an approved future client flow.", route_href: "/team/upload-inbox" }),
    item({ id: "messages", category: "Messaging Readiness", title: "Portal message foundation", status: "needs_review", severity: "info", description: "Messages are portal-only and never SMS, email, DM, or external outreach.", evidence: `${messages.length} Momo message record(s) found for review scope.`, action_hint: "Review portal thread readiness only; do not send external messages.", route_href: "/team/messages" }),
    item({ id: "profile-corrections", category: "Profile Corrections Readiness", title: "Profile correction queue", status: pendingTruth.length ? "needs_owner_confirmation" : "needs_review", severity: pendingTruth.length ? "critical" : "info", description: "Pending business-truth corrections require owner confirmation and Team approval.", evidence: `${pendingTruth.length} Momo pending correction(s) found; no platform update is implied.`, action_hint: "Resolve confirmation requirements before any controlled activation consideration.", route_href: "/team/profile-corrections" }),
    item({ id: "activity-log", category: "Activity Log Readiness", title: "Activity memory", status: activity.length ? "needs_review" : "not_ready", severity: activity.length ? "info" : "warning", description: "Activity Log is a real work-history layer and is not report generation by itself.", evidence: `${activity.length} Momo activity record(s) found for review scope.`, action_hint: "Use only real Veroxa activity as future report input.", route_href: "/team/activity-log" }),
    item({ id: "ai-drafts", category: "AI Draft Readiness", title: "Internal draft preparation", status: drafts.length ? "needs_review" : "not_configured", severity: "warning", description: "AI drafts are internal-only with no raw client visibility and no auto-approval.", evidence: `${drafts.length} Momo AI draft record(s) found for review scope.`, action_hint: "Review drafts internally before any customer-visible use.", route_href: "/team/ai-drafts" }),
